...
const UserDefaultsBridge = registerPlugin('UserDefaultsBridge');

export const saveMealToWidget = async (data: object) => {
  try {
    await UserDefaultsBridge.saveMealData({ data: JSON.stringify(data) });
  } catch (e) {
    console.log('Widget bridge not available');
  }
};